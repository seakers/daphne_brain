...
class Problem(GQLClient):
    ...
    async def get_instruments(self, problem_id=None, attributes=False, measurements=False, capabilities=False):
        ...
        # --> 1. Where
        where_str = await self._where_wrapper([
            await self._where_neighbor('Join__Problem_Instrument', 'problem_id', '_eq', int(problem_id), int)
        ])
        ...
        if attributes is True:
            ...
            """ % await self._where_wrapper([await self._where('problem_id', '_eq', int(problem_id), int)])
        ...
        """ % (where_str, attr_str, meas_str, capa_str)
