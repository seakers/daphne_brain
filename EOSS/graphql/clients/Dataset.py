...
from EOSS.graphql.client import Client
...
class Dataset(Client):
    ...
    async def get_architectures(self, dataset_id=None, costs=True, scores=True):
    ...
    async def clone_architectures(self, source_id, target_id, costs=False, scores=False):
        ...
        print('--> COPYING ARCHITECTURES')
        start_time = timeit.default_timer()
        arch_strings = ''
        loop = asyncio.get_running_loop()
        with ProcessPoolExecutor(max_workers=1) as pool:
            arch_strings = await loop.run_in_executor(pool, self._proc_clone_architecture_strings, architectures, target_id, costs, scores)
