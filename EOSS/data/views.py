...
class ImportData(APIView):
    ...
    def post(self, request, format=None):
        try:
            ...
            print("file pathhhhhhhhhhh", file_path)
...
class CreateProblem(APIView):
    """ Creates a new empty problem and returns the empty dataset 
    
    Request Args:
        problem_name: Name for the new problem
        input_type: Type of inputs (binary or discrete)
        input_num: Number of inputs
        output_num: Number of outputs (typically 2 for Science and Cost)
        formulation: Optional JSON of problem formulation details
        
    Returns:
        problem_id: ID of the newly created problem
        architectures: An empty list that will be populated by the GA
    """
    def post(self, request, format=None):
        try:
            user_info = get_or_create_user_information(request.session, request.user, 'EOSS')
            
            # Extract problem parameters from request
            problem_name = request.data.get('problem_name', 'CustomProblem')
            input_type = request.data.get('input_type', 'binary')  # Default to binary
            input_num = int(request.data.get('input_num', 24))  # Default size for binary string
            output_num = int(request.data.get('output_num', 2))  # Default outputs (Science, Cost)
            formulation = request.data.get('formulation', '{}')  # Optional formulation data
            
            # Generate a unique problem name if needed
            import time
            from datetime import datetime
            timestamp = int(time.time())
            full_problem_name = f"{problem_name}_{timestamp}"
            
            # Create problem directory if it doesn't exist
            user_path = request.user.username if request.user.is_authenticated else 'default'
            problem_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 
                                      "datasets", user_path, full_problem_name)
            os.makedirs(problem_dir, exist_ok=True)
            
            # Create an empty dataset file with proper headers
            filename = "generated_designs.csv"
            file_path = os.path.join(problem_dir, filename)
            
            with open(file_path, 'w', newline='') as csvfile:
                writer = csv.writer(csvfile)
                # Create header based on problem type
                if input_type == 'binary':
                    writer.writerow(['Inputs'] + [f'Output{i}' for i in range(output_num)])
                else:  # discrete
                    writer.writerow([f'Input{i}' for i in range(input_num)] + 
                                   [f'Output{i}' for i in range(output_num)])
            
            # Save formulation data if provided
            if formulation and formulation != '{}':
                formulation_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 
                                            "formulations")
                os.makedirs(formulation_dir, exist_ok=True)
                
                formulation_path = os.path.join(formulation_dir, f"{full_problem_name}.json")
                with open(formulation_path, 'w') as f:
                    f.write(formulation)
            
            # Clear any existing designs for this context
            Design.objects.filter(eosscontext_id__exact=user_info.eosscontext.id).delete()
            
            # Update user context
            user_info.eosscontext.problem = full_problem_name
            user_info.eosscontext.dataset_name = filename
            user_info.eosscontext.dataset_user = request.user.is_authenticated
            user_info.eosscontext.last_arch_id = 0  # Reset architecture ID counter
            user_info.eosscontext.save()
            user_info.save()
            
            # If there was additional metadata to save with the problem, you could do so here
            
            # Notify Java server to start GA if requested
            start_ga = request.data.get('start_ga', False)
            if start_ga:
                # This would be implemented to call your Java server
                # For example:
                # import requests
                # ga_server_url = "http://your-java-server/start-ga"
                # requests.post(ga_server_url, json={
                #     "problem_id": full_problem_name,
            ...
            return Response({
                ...
                "architectures": [],  # Empty list to start
                "message": "Empty problem created successfully"
            })
            
        except Exception as e:
            import traceback
            print("Error creating problem:", traceback.format_exc())
            return Response({
                "success": False,
                "error": str(e)
            }, status=500)