    ...
    def post(self, request, format=None):
        ...
        try:
            ...
            ##################################################################################################################
                ...
                if classify_answer == 'general query':
                    if len(graph_result) == 0:
                        ...
                        user_question = f"This is the history of previous conversations for your context: {self.generate_context(request.data['command'], 'generated')}. These are the cypher query result - {graph_result}. If the cypher query result do NOT match correctly with my question, give the general answer based on the system message. Like for questions on 'what can you do' or 'how do you compute likelihood scores' or 'what can you help with', or something similar or very general questions, answer based on the system message. If the cypher query results match, present cypher query results in a human readable form and give clear, natural, and conversational answers. If asked to provide a pdf or link do NOT say that you cannot provide it. Only present cypher query results without any additional instructions or comments. Do not ask for clarification or specify methods of receiving the document. Do NOT write any fullforms and present information as it is in sentences, give line breaks wherever required to improve formatting. Give the answer with each item on a new line to improve formatting.This is the question: {request.data['command']}"
                        response = client.chat.completions.create(
                            model="gpt-4o",
                            messages=[
                                {"role": "system",
