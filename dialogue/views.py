...
class Command(APIView):
    ...
    def post(self, request, format=None):
        ...
        user_selected_ids = []
        if 'user_selected_ids' in request.data:
            try:
                user_selected_ids = json.loads(request.data['user_selected_ids'])
                print("Combined architecture IDs:", user_selected_ids)
            except json.JSONDecodeError:
                print("Error decoding architecture IDs")

        plot_data = {}
        if 'plot_data' in request.data:
            try:
                plot_data = json.loads(request.data['plot_data'])
            except json.JSONDecodeError:
                print("Error decoding plot data")
        ...
        if False and "is_clarifying_input" in context["dialogue"] and context["dialogue"]["is_clarifying_input"]:
            ...
            # print("command_roles", command_roles)
            ...
            new_dialogue_contexts = self.create_dialogue_contexts()
            dialogue_turn, selected_designs = command_processing.answer_command(processed_command, user_info, context,
                                                                new_dialogue_contexts, request.session, user_selected_ids, plot_data)
            self.save_dialogue_contexts(new_dialogue_contexts, dialogue_turn)
            #         print("dialogue turn", dialogue_turn)
            #     elif max_value > 0.90:
        ...
        print("frontend selecteed designs", selected_designs)

        return Response({'response': frontend_response, 'selected_designs': selected_designs})
...
class Dialogue(APIView):
    ...
    def get(self, request, format=None):
        ...
        # print("last dialogue", last_dialogue[1])
        # print("voice message", last_dialogue[1].voice_message)
        # print("visual message type", last_dialogue[1].visual_message_type)
        # print("visual message", last_dialogue[1].visual_message)
        # print("writer", last_dialogue[1].writer)
        # print("last dialogue", last_dialogue[2])
        # print("voice message", last_dialogue[2].voice_message)
        # print("visual message type", last_dialogue[2].visual_message_type)
        # print("visual message", last_dialogue[2].visual_message)
        # print("writer", last_dialogue[2].writer)
        dialogue_list = [
            {
