...
# from EOSS.dialogue.gpt_chat import main
# from dialogue.nn_models import nn_models
from EOSS.dialogue.dialogue_classifier.dialogue_manager import main
...
#     # evaluate loaded model on test data
#     result_logits = loaded_model.predict(x)
#     print("result logits", result_logits)
...
def answer_command(processed_command, user_info: UserInformation,
                   context, new_dialogue_contexts, session, user_selected_ids, plot_data):
    # Create a DialogueContext for the user to fill
    # answer = command(processed_command, user_info, context,
    #                  new_dialogue_contexts, session)
    data = {}
    data['now'] = datetime.datetime.utcnow()
    print("user info", user_info)
    print("user info eosscontext", user_info.eosscontext)
    data['session_key'] = session.session_key
    data['designs'] = user_info.eosscontext.design_set.order_by('id').all()
    data['problem'] = user_info.eosscontext.problem
    answer, selected_designs = main(processed_command, user_info, context, data, new_dialogue_contexts, session, user_selected_ids, plot_data)
    print(answer)
    ...
    return dialogue_history, selected_designs
