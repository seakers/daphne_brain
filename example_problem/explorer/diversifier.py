import datetime
from example_problem.models import Design, ExampleContext
import json
import random
...
def activate_diversifier(examplecontext: ExampleContext):
    if not examplecontext.activecontext.check_for_diversity:
        return

    # 1. Compute the pareto front
    dataset = Design.objects.filter(examplecontext_id__exact=examplecontext.id).all()
    json_dataset = []
    ...
    if is_considerably_larger:
        ...
        DialogueHistory.objects.create(user_information=examplecontext.user_information,
        ...
        async_to_sync(channel_layer.send)(examplecontext.user_information.channel_name,
                                          {
                                              'type': 'active.message',
                                              'message': message
                                          })
